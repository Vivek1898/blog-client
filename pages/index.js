...
function Home() {
  ...
  const searchItems = (searchValue) => {
    ...
    if (searchInput !== "") {
      const filteredData = latestPosts.filter((item) => {
        return item.title.toLowerCase().includes(searchInput.toLowerCase());
      });
      setFilteredResults(filteredData);
    } else {
      setFilteredResults(latestPosts);
    }
  };
  ...
  return (
    <div className="text-black">
      ...
      <div id="hero" class=" lg:flex items-center">
          <div class="px-5 sm:px-10 md:px-10 md:flex lg:block lg:w-1/2 lg:max-w-3xl lg:mr-8 lg:px-20">
            <div class="md:w-1/2 md:mr-10 lg:w-full lg:mr-0">
              <h1 class="text-4xl xl:text-5xl font-black md:leading-none xl:leading-tight">
          ...
          <div class="mt-6 w-full flex-1 lg:mt-0">
            <img class="" src="https://images.unsplash.com/photo-1505330622279-bf7d7fc918f4?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=870&q=80" />
          </div>
        </div>
      ...
      <main class="flex w-full h-full shadow-lg ">
       ...
       {searchInput.length > 1 ? (filteredResults.map((post) => (
            <section>
...
{/* 
        ...
        <section class="flex flex-col pt-3 w-4/12 bg-gray-50 h-full overflow-y-scroll">
          ...
          {latestPosts.map((post) => (
            ...
            <li class="flex justify-between py-5 border-b px-3 transition hover:bg-indigo-100">
              <div>
                <div class="flex h-12 w-12 rounded-full overflow-hidden">
                  <img src={post.featuredImage?.url || "images/default.jpeg"} loading="lazy" class="h-full w-full object-cover" />
                </div>
                <a href="#" class="items-center">
                  ...
                  <div class="text-md italic text-gray-400">{post.content}</div>
                  <div className="flex pt-2">
                  {categories.map((post) => (
                    <Link href={`/category/${post.slug}`} key={post._id}>
                    <button type="button" class="mr-2 text-blue-700 border border-blue-600 hover:bg-blue-700 hover:text-white focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-full text-sm py-1 px-5 text-center inline-flex items-center dark:border-blue-500 dark:text-blue-500 dark:hover:text-white dark:focus:ring-blue-800">
                      <p>{post.name}</p>
                    </button>
                    </Link>
                    ))}
              ...
              <p class="text-md text-gray-400">{post.likes}</p>
            </li>
            ))}
